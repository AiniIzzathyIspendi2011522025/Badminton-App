                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            ...
                            <a style="float:right;" href="#" onclick="deleteVenue({{ $venues->id }})">
                                <button class="btn btn-danger">Hapus Lapangan</button>
                            </a>
                            <a style="float:right;margin-right:5px;" href="{{ route('owner.venue.edit', $venues->id) }}">
                                <button class="btn btn-success">Edit Lapangan</button>
                            </a>
                            <a style="float:right; margin-right:5px;"
                                href="{{ route('owner.venue.field.create', $venues->id) }}">
                                <button class="btn btn-success">Tambah Lapangan</button>
                            </a>
                            {{-- <button style="float:right; margin-right:5px;" class="btn btn-primary" data-toggle="modal" data-target="#imb">Lihat Surat
                            Izin</button> --}}
                        </div>
                        <!-- /.card-header -->
                        <div class="modal fade" id="imb" tabindex="-1" role="dialog"
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <figure>
                                            <img src="{{ asset('images/imb/' . $venues->imb) }}" alt="Trulli"
                                                style="width:100%">
                                        </figure>
                                    </div>
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-12">
                                    <a href="#"
                                        class="btn btn-success btn-icon icon-left btn-lg btn-block mb-4 d-md-none"
                                        ...
                                        <div class="ticket-content">
                                            <div class="ticket-header">
                                                <div class="ticket-sender-picture img-shadow">
                                                    <img src="{{ asset('templates/img/avatar/avatar-5.png') }}"
                                                        alt="image">
                                                </div>
                                                <div class="ticket-detail">
                                                    ...
                                                    <div class="ticket-info">
                                                        <div class="font-weight-600">{{ $venues->owner->first_name }}
                                                            {{ $venues->owner->last_name }}</div>
                                                        <div class="bullet"></div>
                                                        <div class="text-primary font-weight-600">
                                                            {{ $venues->owner->handphone }}
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="ticket-description">
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-6">
                                                        <div class="mb-1">
                                                            @if ($venues->isMembership)
                                                                <p>
                                                                    <strong>Membership Aktif: Diskon untuk member
                                                                        {{ $venues->membership_discount }} %
                                                                    </strong>
                                                                </p>
                                                                <p>
                                                                    <strong>Harga
                                                                        Membership:{{ Helper::rupiah($venues->membership_price) }}
                                                                    </strong>
                                                                </p>
                                                            @endif
                                                        </div>
                                                        ...
                                                        @if ($venues->dp_percentage != null)
                                                            <p><b>- Pembayaran dengan DP sebesar
                                                                    {{ $venues->dp_percentage }} %</b>
                                                                <br>
                                                ...
                                                <div class="row">
                                                    <div class="col-lg-12">
                                                        <table class="table table-striped dataTables-field"
                                                            id="table-2">
                                                            <thead>
                                                                <tr>
                                                                    <th>Nama Lapangan</th>
                                                        ...
                                                        <div class="badges">
                                                            @foreach ($venues->OpeningHour as $open)
                                                                @if ($openingHour->day_id == $open->day_id)
                                                                    @if ($open->status == 2)
                                                                        <span
                                                                            class="badge badge-info">{{ $open->Hour->hour }}</span>
                                                                    @else
                                                                        <span
                                                                            class="badge badge-secondary">{{ $open->Hour->hour }}</span>
                                                                    @endif
                                                                @endif
                                                            @endforeach
                                                        </div>
                ...
                columnDefs: [{
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        return `` +
                            ...
                            ` <a class="btn btn-icon icon-left btn-outline-dark btn-sm" href="/owner/field/` +
                            data +
                            `/edit"><i class="fas fa-edit"></i>Edit</a>` +

                            ` <a class="btn btn-icon icon-left btn-outline-dark btn-sm" onclick="confirmdelete(` +
                            data +
        ...
        function deleteVenue(id) {
            swal({
                ...
                function(isConfirm) {
                    if (isConfirm) {
                        swal("Data Terhapus!", "Data tersebut telah terhapus", "success");
                        $('#deleted_venue').attr('action', "{{ route('owner.venue.index') }}/" + id);
                        $('#deleted_venue').submit();
                    } else {
                        swal("Batal", "Anda batal menghapus data", "error");
                    }
                });
